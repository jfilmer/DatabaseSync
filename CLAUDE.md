...
> ⚠ **Lookback is applied TWICE on the first run.** Observed: `using lookback of 2160h from now: 2026-05-11` then `Loading rows changed since 2026-02-10`. The second block's comment claims it "only applies lookback if lastSyncTime came from history", but its condition never tests that. Pre-existing, and it errs safe (syncs more, not less) — just do not be surprised that the first window is double what you configured.
...
> ⚠ **Unrelated, still open — `_sync_history.rows_deleted` is ALWAYS 0 on PG→PG two-phase profiles.** Measured 2026-08-09: **max `rows_deleted` = 0 across all 2,187 rows** of profile 12, while the same run's summary reported 14 deletes and `rmp.audit_log` demonstrably went 286 → 272. Two-phase sync writes each table's history row at the end of phase 1, before the phase-2 delete pass runs, so deletes are never recorded per-table. Pre-existing and **not** introduced by the `skipped` work — the 2,187-row history proves it — but it is the same "history understates reality" family, so do not trust `rows_deleted` in history for any PG→PG mirror.
